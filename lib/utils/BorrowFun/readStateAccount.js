...
import cbs_protocol_idl from "../../idls/cbs_protocol.json";
import { convert_from_wei } from "../../helpers/constants/common";
import { config } from "../../helpers/constants/cbs_constants";
...
export const readStateAccount = async (wallet) => {
  try {
    ...
    const programId = new PublicKey(cbs_protocol_idl.metadata.address);

    const program = new anchor.Program(cbs_protocol_idl, programId);
